...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="author" select="'B. Cristina Pelayo García-Bustelo'"/>

	<xsl:template match="articles">
		<xsl:text disable-output-escaping='yes'>&lt;!DOCTYPE html></xsl:text>
		<html lang="es">
			<head>
				<meta name="description" content="Ejercicio resuelto 4"/>
			...
			<body>
				<header>
					<h1>EJERCICIO 4</h1>
					<h2>
						Artículos de revista de un autor (<xsl:value-of select="$author"/>) ordenados por fecha
					</h2>
				</header>
				<main>
					<xsl:for-each select="article">
						<xsl:sort order="ascending" select="@year"/>
						<xsl:if test="contains(authors, $author)">
