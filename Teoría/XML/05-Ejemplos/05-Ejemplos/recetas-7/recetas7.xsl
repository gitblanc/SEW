...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="milk" select="'leche'"/>
	<xsl:variable name="cheese" select="'queso'"/>
	
	<xsl:template match="cooking-recipes">
		<xsl:text disable-output-escaping='yes'>&lt;!DOCTYPE html></xsl:text>
		<html lang="es">
			<head>
				<meta name="description" content="Ejercicio resuelto 7"/>
			...
			<body>
				<header>
					<h1>EJERCICIO 7</h1>
					<h2>Menú sin leche ni queso</h2>
				</header>
				<main>
					<xsl:for-each select="cooking-recipe">
						<xsl:variable name="ingredients-lower-case" select="translate(ingredients, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')"/>
						<xsl:if test="not(contains($ingredients-lower-case, $milk)) and not(contains($ingredients-lower-case, $cheese))">
